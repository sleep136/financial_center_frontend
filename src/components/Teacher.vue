...
async function fetchReservationData() {
  ...
  try {
    const response: AxiosResponse<ApiResponse<ReservationItem>> = await request.get('/teacher/reimbursement', {
      params: {
...
async function fetchLaborData() {
  loading.labor = true
  try {
    const response: AxiosResponse<ApiResponse<LaborItem>> = await request.get('/teacher/labor_cost', {
      params: {
        work_id: formData.work_id || '',
        filter_state: formData.filter_state || 1,
      }
...
watch(activeTab, (newTab) => {
  if (newTab === '报销明细' && formData.work_id) {
    fetchReservationData()
  } else if (newTab === '劳务明细' && formData.work_id) {
    fetchLaborData()
  }
})
